    ...
    >
      <div className="max-w-6xl mx-auto px-4 sm:px-6 md:px-10 h-16 flex items-center">
        ...
        {/* No toggle, no hidden state — the links themselves are always
            ...
            silently fail on a phone. */}
        <nav
          className="no-scrollbar flex-1 flex items-center gap-1 overflow-x-auto"
          style={{
            scrollbarWidth: 'none',
            WebkitOverflowScrolling: 'touch',
          }}
        >
          {LINKS.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className="font-mono text-xs sm:text-sm whitespace-nowrap px-3 py-2.5 rounded-md shrink-0"
              style={{ color: 'var(--text-dim)', touchAction: 'manipulation', minHeight: '40px' }}
            >
              {link.label}
            </a>
          ))}
